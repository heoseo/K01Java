...
public class SelectSQL extends ConnectDB{
	...
	/*
		* ResultSet클래스
			...
			: 결과로 반환된 레코더의 마지막까지 next()메소드를 통해 확인 후 반복하면서 추출.
			 - getXXX()계열의 메소드
			 - 오라클의 자료형이
			 	number타입 : getInt()
...
//			String query = "SELECT id, pass, name, regidate from member";
			...
			System.out.println("id\tpw\tname\tregidate\tregidate2\tregidate3");
			...
			while(rs.next()) {
				...
				0000-00-00 00:00:00형태로 시간까지 출력할 수 이싿.
				...
				String regidate4 = rs.getString("d1"); 	// 뱔칭 그대로 사용
