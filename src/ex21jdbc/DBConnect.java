...
public class DBConnect {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*
			2-1] DriveManager클래스의 메소드를 통해 오라클에 연결을 시도한다.ㅏ
			...
			if(con!= null) {
				...
				해당 객체는 주로 정적 쿼리를 실행할때 사용된다.
				 */
				Statement stmt = con.createStatement();
				
				/*
				5] Statement객체를 이용하여 Oracle로 작성된 쿼리문을 전송하고 실행한다.
				...
				/*
				6] 자원반납
					: 모든 작업을 마친 후에는 메모리 절약을 우이해 연결했던 자원을 반납한다.
				 */
				rs.close();
				stmt.close();
				con.close();
