...
//	}
	...
	public으로 선언된 메소드를 통해서 간접 호출해야 한다.
	상속관계라면 protected으로 선언된 메소드로도 호출할 수도 있다.
	 */
	...
	// 해당 메소드를 protected으로 선언하면 상속관꼐의 하위 객체에서 호출할 수 있다.
	public void calculateFunc() {
		keyboardTouch();
		calculate();
	}
	// 해당 메소드는 내부에서만 접근되므로 호출 순서를 지켜 실행할 수 있도록 캡슐화하여 정의하였다.
}
...
class NotebookComputer extends Computer{
	...
	/*
	부모생성자 호출을 위해 사용하는 super()는 생략은 가능하지만 앞에 다른 문장이 먼저오면
	오류가 발생하므로 주의해야 한다.
	...
	public NotebookComputer(String name, int initCharge) {
		...
		부모클래스에서 인자생성자를 정의하였기 떄문에 기본생성자는 생성되지 않기 때문이다.
	...
	public void movigCal() {
		...
		// private으로 선언되었으므로 상속받은 자식클래스에서도 접근 불가능.
//		keyboardTouch();	//[에러발생!]
//		calculate();		//[에러발생!]
...
/*
태블릿 노트북 컴퓨터 : 컴퓨터의 기본기능 및 휴대이용 기능 그리고 펜으로 드로잉할수 있는 기능까지 추가된 클래스.
 */
class TabletNotebook extends NotebookComputer{
	...
	// 태블릿 사용으 ㄹ위해 배터리양 체크 및 등록된 펜인지 확인
	public void write(String penInfo) {
		...
		/*
		A.compareTo(B)
			: 문자열 A와 B를 비교하여 0이 반환되면 같은 문자열로 판단하고 -1혹은 1이 반환되면 서로 다른 문자열로 
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		/*
		배터리가 소모되어 사용할 수 없는 상태가 되는지 확인하기 위해 movingCal()을 여러 번 호출한다.
		 */
