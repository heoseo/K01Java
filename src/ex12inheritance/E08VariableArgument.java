...
가변인자(Variable Argument)
	: 메소드 오버로딩을 대체할 수 있는 문법으로 하나의 메소드로 여러 가지 호출형식을 대체 할 수 있다.
	[형식]
		메소드명(자료형 ... 매개변수명){
			실행문;
		}
	 - 필요에 따라 매개변수를 가변적으로 조장할 수 있는 문법으로 JDK1.5 부터 지원한다.
	 - 매개변수는 여러 개를 한꺼번에 받아야 하므로 배열로 반환된다.
 */
...
//	}
	...
	// 가변인자를 통한 메소드 정의
	/*
	가변인자를 통한 메소드 정의시 매개변수가 하나의 타입일 때만 가능하다.
