...
/*
instanceof 연산자
	: ㅇ인스턴스 변수가 어떤 타입의 변수인지를 판단하는 연산자로
	형변환이 가능하면(즉, 상속관계가 있으면)true, 아니면 false를 반환한다.
...
class GoldPaperBox extends PaperBox{
	public void goldWrap() {
		System.out.println("GoldPaterBox로 포장합니다.");
	}
}
...
public class E09InstanceOf {
	...
	/*
	여기서 instnace 연산자를 통해 형변환 여부를 판단하는 이유는
	전달된 매개변수를 최상위 클래스인 Box타입으로 받고 있기 때문이다.
	- Box 타입으로 인자를 받게 되면 자동으로 업캐스팅(자동형변환)이 되기 때문에 자식클래스의 멤버메소드를 호출할 수 없다.
	- 이때 자식의 멤버메소드를 호출하기 위해서 다운캐스팅(명시적형변환)이 필요하다.
	- 따라서 전달된 인자를 각각의 타입으로 형변환이 가능한지 판단한 후 
	다운캐스팅을 진행하고 각 틀래스의 멤버메소드를 호출할 수 있게된다
	 */
...
//		double num2 = 1;
	...
	public static void main(String[] args) {
		...
		wrapBox(box2);	// GoldBox타입의 매개변수
		wrapBox(box3);	// GoldPaperBox타입의 매개변수
		
		String str = new String("나는 누구?");
//		wrapBox(str);	// String객체와는 상속관계가 없으므로 호출 불가.
	}
