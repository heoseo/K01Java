...
/*
쓰레드의 우선순위
	- setPriority(우선순위) : 쓰레드의 우선순위를 지정할 떄 사용
...
class MessageSendingThread extends Thread{
	...
	public void run() {
		for( int i = 1; i <= 10000; i++) {
			...
			sleep()메소들르 사용하면 실행 중 잠깐씩 블럭상태로 전환되어
			웅선순위가 낮은 쓰레드도 가끔 먼저 실행될 수 있는 상황이 생긴다.
			 */
			try {
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		/*
		1단계 실행
			: 우선순위를 부여하지 않은 상태로 쓰레드를 생성하면 동일한 우선순위 5를 부여받는다.
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		2단계 실행
			...
			: 우선순위는 1~10 까지 부여할 수 있고 높을수록 우선순위는 높아진디ㅏ.
			※ 단, 우선순위가 높다고 무조건 항상 먼저 실행되지는 않는다.
			전체적인 확률이 높아져 먼저 실행될 가능성이 높아지는것 뿐이다.
		 */
