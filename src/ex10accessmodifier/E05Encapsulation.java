...
/*
	* 캡슐화(Encapsulation)
		: 여러 가지 비즈니스 로직을 하나로 묶는다는 의미와 
		    업무의 순서까지 고려한 형태의 로직을 구성한다는 의미를 가지고 있다.
		: 즉, 관련있는 메소드르 ㄹ하나의 클래스로 묶는 것을 말한다.
 */
...
// 업무의 순서를 고려하여 관련된 위 4개의 메소드를 하나의 클래스로 정의하였다.
...
class EncapsulLogic{
	...
	void doProcess() {
		memberRegist.doMemberRegist();				// 1
		autoLogin.doAtuotoLogin();					// 2
		congratulationPoint.doCongratulationPoint();// 3
		firstLoginEvent.doFirstLoginEvent();		// 4
	}
}
