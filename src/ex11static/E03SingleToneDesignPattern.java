...
// Math, Calendar : new로 객체 생성 불가능, singletone이라서.

/*
	* 싱글톤 디자인 패턴
		: 클래스를 설계하는 디자인패턴의 하나로써 하나의 인스턴스, 즉 하나의 메모리를 생성해
		이를 공유해서 사요하고자 할 때 사요하는 패턴이다.
		- 작성방법
		 1) 생성자의 접근지정자를 private으로 정의한다.
		 2) 외부클래스에서 생성자를 접근할 수 없으므로 new를 통해 객체 생성은 불가능해짐.
...
class SingleTone{
	...
	// JVM(자바가상머신)에 의해 프로그램이 시작될 때 정적변수는 미리 메소드 영역에 생성되어 사용할 준비를 마친다.
	private static SingleTone single = new SingleTone();
	
	// 생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 없으므로
	// new 연산자를 통해서 는 객체를 생성할 수 없게 된다.
	private SingleTone() {	// private => new로 객체 생성 안됨.
		// 기본생성자
	}
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		// 일반적인 방식의 객체생성이므로 객체를 생성할 때 마다 새로운 참조값(주소)을 할당받는 인스턴스가 된다.
...
//		SingleTone st1 = new SingleTone();	// [에러발생!] 생성자가 private이므로 외부에서 접근 불가능.
		
		
		/*
		생성자에 접근할 수 없는 대신 getInstance() 메소드를 통해서만 객체의 참조값을 반환받을 수 있다.
		반환받을 때는 항상 같은 객체를 반환받는다.
		 */
