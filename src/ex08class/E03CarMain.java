...
class Car{
	...
	String carModel;
	int carYear;
	Human owner;/* Human 클래스를 기반으로 하는 멤버변수로
					초기화를 위해 이름, 나이, 에너지 세 가지의 값이 필요함.
					이와 같이 클래스를 구성할 때 또 다른 객체를 멤버변수를 선언할 수 있다.
				*/
...
//	초기화 메소드 (1)
	void initialize() {			// 객체의 초기화를 담당하는 메소드로 아래는 항상 같은 값으로만
		carModel = "람보르기니";	// 초기화 된다는 한계가 있음.
		carYear= 2017;
		owner = new Human();
		owner.name = "캡틴로저스";
		owner.age = 30;
...
//	초기화 메소드 (2)
	void initialize(String model, int year, String name, int age, int energy) {
		carModel = model;		// 위와 동일한 이름의 함수로 오버로딩에 의해 정의된 초기화 메소드.
		carYear = year;			// 초기화를 진행하는 역할은 동일하나, 
								// 매개변수를 통해 다양한 형태의 객체를 초기화할 수 있다.
		// 또 다른 객체가 멤버변수로 정의되었으믈, 객체를 생성한 후 각각의 멤버변수를 초기화해야 한다.
		owner = new Human();	
		owner.name = name;
		owner.age = age;
		owner.energy = energy;
	...
	void showCarInfo() {		// 해당 객체의 정보를 디스플레이 해줌.
...
//		car1.drive();	-> [에러발생!]
//							자동차 소유주를 표현하는 Human 객체가 초기화되지 않은 상태엥서
//							출력을 시도하여 런타임에러가 발생된다.
		
		System.out.println("[초기화메소드 호출 후");
		car1.initialize();		// 고정된 내용으로 초기화가 진행됨.
		car1.drive();
		car1.showCarInfo();
